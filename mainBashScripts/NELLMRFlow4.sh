#file code  NELLMRFlow4 (M-E-R) dengan jumlah worker 5
...
do 
	...
	python 15.addConceptLabeltoValid.py ../DataMRCombination/outputMateIte$i/CandinputKG$i.txt ../TBOXesNELL/NELLInstClassString.txt ../PrepareDataForWorker/NewForTransC.txt  
	...
	cd ..
	mv DataMRCombination/forMate.txt DataMRCombination/input4Rules$i.txt 
	...
	$(source ./NELLAPR4.sh ; APR $numPartition $i)
	#numCorrect=$(python ../ScriptMRCombination/countLine.py outputRLIte$i/ValidLabels.txt)

	cd ~/sharedscratch/CARRELFlow2TransC/CARRELTransC/DataMRCombination/
	mv outputRLIte$i/NELLKG$i.txt ~/sharedscratch/CARRELFlow2TransC/CARRELTransC/DataMRCombination/
	cd ..
